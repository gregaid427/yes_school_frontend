...
const AddExpense = () => {
  ...
  const { CreateExpense, fetchAllExpense, deleteExpensees,fetchexpensehead,updateExpense } = expense;
  ...
  useEffect(() => {
    if (updateExpense?.success == 0) {
    }
    if (updateExpense?.success == 1) {
      setVisible2(false);
    }
  }, [updateExpense]);
  ...
  return loader ? (
    ...
    <DefaultLayout>
      ...
      >
        <ExpenseFormModalEdit info={info} head={fetchexpensehead} close={setVisible2} />
      </Dialog>
      <Dialog
        visible={visible1}
        position={'top'}
