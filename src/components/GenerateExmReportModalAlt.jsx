...
import {
  ...
  GenerateAllReportAction,
  ResetClassReport,
  resetcreateexam,
  ResetGenerateAllReport,
...
const GenerateExmReportModalAlt = (props) => {
  console.log(props);
  ...
  const { createexam,FetchexamSubject,GenerateAllReport } = exam;
  ...
  useEffect(
    ...
    useEffect(() => {
      if (GenerateAllReport?.success == 0) {
      }
      if (GenerateAllReport?.success == 1) {
       
props.close(false)
dispatch(ResetGenerateAllReport())
      }
    }, [GenerateAllReport]);
  ...
  function handleGetClassreport(val) {
    ...
    dispatch(GenerateAllReportAction(data));
  }
...
export default GenerateExmReportModalAlt;
