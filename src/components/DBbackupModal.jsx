...
import { createBackupAction, resetcreateBackup } from '../redux/slices/sessionSlice';

const DBbackupModal = (props) => {
  console.log(props);
  ...
  const { createBackup } = session;

  useEffect(() => {

    if (createBackup?.success == 1) {
      let url = createBackup?.link;
      window.open(url);
          ...
          <div style={{ display: display == 1 ? 'block' : 'none' }}>
            <div className="w-full mb-4 sm:w-2/2">
              <label
                ...
                htmlFor=""
              >
                Backup Database at Current DateTime ?
              </label>
            </div>
            {/* <div className="w-full mb-4 sm:w-2/2">
              <label
